import { useState } from "react";
import { useSearchParams } from "react-router-dom";
import { useAuth } from "@/contexts/auth-context";
import { CheckCircle2, Loader2 } from "lucide-react";
import { Alert, AlertDescription } from "@/components/ui/alert";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { api } from "@/services/api";

function GitHubIcon({ className }: { readonly className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden
    >
      <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
    </svg>
  );
}

export function GitHubLinkCard() {
  const { user } = useAuth();
  const [searchParams] = useSearchParams();
  const [isLinking, setIsLinking] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const justLinked = searchParams.get("github_linked") === "true";
  const hasGitHub = user?.githubId != null;
  const isEmailUser = user?.authProvider === "email";

  const handleLinkGitHub = async () => {
    setError(null);
    setIsLinking(true);

    try {
      const { redirectUrl } = await api.auth.linkGitHub();
      globalThis.location.href = redirectUrl;
    } catch {
      setError("Failed to initiate GitHub linking. Please try again.");
      setIsLinking(false);
    }
  };

  if (!isEmailUser && !justLinked) {
    return null;
  }

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <GitHubIcon className="h-5 w-5" />
          GitHub Account
        </CardTitle>
        <CardDescription>
          Link your GitHub account to enable Git-based deployments
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {justLinked && (
          <Alert>
            <CheckCircle2 className="h-4 w-4 text-emerald-500" />
            <AlertDescription>
              GitHub account linked successfully!
            </AlertDescription>
          </Alert>
        )}

        {error && (
          <Alert variant="destructive">
            <AlertDescription>{error}</AlertDescription>
          </Alert>
        )}

        {hasGitHub ? (
          <div className="flex items-center gap-3">
            <GitHubIcon className="h-5 w-5 text-muted-foreground" />
            <span className="text-sm font-medium">{user?.githubLogin}</span>
            <Badge variant="secondary">Connected</Badge>
          </div>
        ) : (
          <Button
            onClick={handleLinkGitHub}
            variant="outline"
            disabled={isLinking}
          >
            {isLinking ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" aria-hidden />
                Connecting...
              </>
            ) : (
              <>
                <GitHubIcon className="mr-2 h-4 w-4" aria-hidden />
                Link GitHub Account
              </>
            )}
          </Button>
        )}
      </CardContent>
    </Card>
  );
}
